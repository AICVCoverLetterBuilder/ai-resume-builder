...
const WAREHOUSE_TITLE_CORE_RE =
  /(?:warehouse\s*(?:employee|worker|operator|associate)?|\bwarehouse\b|वेयरहाउस(?:\s*(?:कर्मचारी|वर्कर))?|موظف(?:ة)?\s*مستودع|кладовщ(?:иц[аыуе]|ик(?:а|ом|у)?)?|skladistar(?:ka)?|magacioner(?:ka)?|lagerist(?:kinja)?|radni(?:k|ca)\s+u\s+(?:magacinu|skladistu)|u\s+skladistu)/iu;
...
const WAREHOUSE_DUTY_CUE_RE =
  /(?:warehouse|वेयरहाउस|गोदाम|magacin|skladist|skladišt|incoming\s+goods|माल|товар|склад|кладов|بضائع|مستودع)/iu;
