...
const DURATION_EXPRESSION_RES: RegExp[] = [
  ...
  new RegExp(
    String.raw`(?<!\p{L})с\s+(?:общим\s+)?опытом\s+около\s+(?:\p{L}+(?:\s+с\s+половиной)?|${NUM})\s+лет(?!\p{L})`,
    'giu',
  ),
  new RegExp(
    String.raw`(?<!\p{L})(?:около|примерно)\s+(?:\p{L}+(?:\s+с\s+половиной)?|${NUM})\s*(?:лет|года|год)(?:\s+(?:трудового\s+|общего\s+)?опыта)?(?!\p{L})`,
    'giu',
  ),
  new RegExp(
    String.raw`(?<!\p{L})(?:один|одного|одна|два|двух|три|трёх|трех|четыре|четырёх|четырех|пять|пяти|шесть|шести|семь|семи|восемь|восьми|девять|девяти|десять|десяти|${NUM})(?:\s+с\s+половиной)?\s*(?:лет|года|год)(?:\s+(?:трудового\s+|общего\s+)?опыта)(?!\p{L})`,
    'giu',
  ),
  new RegExp(
    String.raw`(?<!\p{L})${NUM}\s*(?:лет|года|год)(?:\s+(?:трудового\s+|общего\s+)?опыта)(?!\p{L})`,
    'giu',
  ),
