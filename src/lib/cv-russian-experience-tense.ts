...
const PRESENT_3SG =
  /(?:проверяет|обновляет|поддерживает|координирует|согласовывает|созда[её]т|адаптирует|подготавливает|готовит|обеспечивает)/iu;
const PAST_FEMALE =
  /(?:проверяла|обновляла|поддерживала|координировала|согласовывала|создавала|адаптировала|подготавливала|готовила|обеспечивала|выполняла|работала)/iu;
const PAST_MALE =
  /(?:проверял|обновлял|поддерживал|координировал|согласовывал|создавал|адаптировал|подготавливал|готовил|обеспечивал|выполнял|работал)(?!а)/iu;
