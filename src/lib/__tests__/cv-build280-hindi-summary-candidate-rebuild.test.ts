...
describe('build 280 Hindi Summary persisted-state rebuild', () => {
  it('exposes runtime-280 revision markers from executing modules', () => {
    expect(SUMMARY_PIPELINE_REVISION).toBe('summary-runtime-280-v1');
    expect(SUMMARY_BUILDER_REVISION).toBe('entry-owned-candidate-rebuild-v2');
    expect(SUMMARY_UNIT_SPLITTER_REVISION).toBe('hindi-sentence-slot-split-v2');
    expect(SUMMARY_GROUNDING_REVISION).toBe('entry-owned-grounding-v2');
    expect(SUMMARY_DURATION_FINALIZER_REVISION).toBe('duration-double-pass-v2');
  });
